...
function barrido(){
    lines.forEach(function(line) {
        menu(line.startX, line.startY, line.endX, line.endY, line.option);
    });
}
...
canvas.addEventListener("mousemove", function(event) {
    if (isDrawing) { // Verifica si se está dibujando actualmente
      clearCanvas()
      menu(startX, startY, event.offsetX, event.offsetY, option); // Dibuja la figura previsualizada
    }
  });
...
function menu(x1, y1 , x2 , y2, opt){
  if(x1!=null && y1!=null && x2!= null && y2 != null ){
    if(opt == 'line'){
        ...
        drawPoligon(x1, y1 , x2 , y2, 5)
      }else if(opt == 'hexagon'){
        drawPoligon(x1, y1 , x2 , y2, 6)
      }else if(opt == 'heptagon'){
        drawPoligon(x1, y1 , x2 , y2, 7)
      }else if(opt == 'octagon'){
        drawPoligon(x1, y1 , x2 , y2, 8)
      } 
  }
}
...
function drawSquare(startX, startY, endX, endY) {
  ...
  Bresenham(squareX, squareY, squareX + squareSize, squareY);
  Bresenham(squareX + squareSize, squareY, squareX + squareSize, squareY + squareSize);
  Bresenham(squareX + squareSize, squareY + squareSize, squareX, squareY + squareSize);
  Bresenham(squareX, squareY + squareSize, squareX, squareY);
}
